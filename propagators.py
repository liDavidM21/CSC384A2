    ...
    if not newVar:
        ...
        for c in csp.get_all_cons():
            if(c.get_n_unasgn() == 1):
                ...
                result, lst = FCCheck(c,X)
                for i in lst:
                    purned_list.append(i)
                if(not result):
                    return False, purned_list
        return True, purned_list
    else:
        purned_list = []
        for c in csp.get_cons_with_var(newVar):
            if(c.get_n_unasgn() == 1):
...
def prop_GAC(csp, newVar=None):
    ...
    if not newVar:
        ...
        result, lst = GAC_enforce(csp, GAC_queue)
        return result, lst
    else:
        GAC_queue = []
        for c in csp.get_cons_with_var(newVar):
...
def ord_mrv(csp):
    ...
    for i in csp.get_all_vars():
        if(count > i.cur_domain_size()):
            count = i.cur_domain_size()
            minimum = i
    return minimum
